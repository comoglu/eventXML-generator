...
def generate_synthetic_waveforms(event_file, inventory_dir, output_file, channel_list=None):
    ...
    available_channels = set(channel_list) if channel_list else get_available_channels(inventory)

    model = TauPyModel(model="iasp91")
    phases = ["P", "S", "PP", "SS"]  # Removed "Love" as it's not a valid phase name for TauPyModel
...
def create_synthetic_traces(origin, magnitude, inventory, model, phases, available_channels, network, station):
    ...
    for component in ['Z', 'N', 'E']:
        ...
        noise = create_realistic_noise(npts, sampling_rate)
        data += noise

        data = bandpass(data, 0.01, 25, sampling_rate, corners=4)
        ...
        tr.stats.back_azimuth = (azimuth + 180) % 360
...
def create_surface_waves(t, distance_deg, magnitude, component):
    rayleigh_velocity = 3.0 + 0.5 * np.random.rand()  # Add some randomness
    love_velocity = 3.5 + 0.5 * np.random.rand()

    rayleigh_arrival_time = distance_deg * 111.19 / rayleigh_velocity
    love_arrival_time = distance_deg * 111.19 / love_velocity

    rayleigh_amplitude = 10**(magnitude - 5.5) * np.exp(-0.0005 * distance_deg)
    love_amplitude = 0.8 * rayleigh_amplitude

    rayleigh_freq = 0.1 - 0.05 * np.exp(-distance_deg / 1000)  # Frequency decreases with distance
    love_freq = 0.08 - 0.04 * np.exp(-distance_deg / 1000)

    rayleigh_wave = rayleigh_amplitude * np.sin(2*np.pi*rayleigh_freq*(t-rayleigh_arrival_time)) * \
                    np.exp(-(t-rayleigh_arrival_time)/100) * \
                    (t > rayleigh_arrival_time)

    love_wave = love_amplitude * np.sin(2*np.pi*love_freq*(t-love_arrival_time)) * \
                np.exp(-(t-love_arrival_time)/120) * \
                (t > love_arrival_time)
...
def create_phase_data(t, arrival, magnitude, distance_deg, component):
    ...
    f = 1.0 + 2.0 * np.exp(-distance_deg / 1000)  # Frequency decreases with distance
    tau = (t - arrival_time) * f * np.pi
    amplitude = 10**(magnitude - 4.5) * np.exp(-0.001 * distance_deg)

    if arrival.name in ['P', 'PP']:
        wave = amplitude * (1 - 2 * tau**2) * np.exp(-tau**2) * (1 if component == 'Z' else 0.2)
    elif arrival.name in ['S', 'SS']:
        wave = amplitude * 1.5 * tau * np.exp(-tau**2) * (0.2 if component == 'Z' else 1)
    else:
        wave = np.zeros_like(t)

    return wave * (t >= arrival_time)

def apply_attenuation_and_scattering(data, distance_deg, sampling_rate):
    def Q(f):
        return 150 + 15 * f  # Increased Q values for more realistic attenuation

    f = np.fft.fftfreq(len(data), 1/sampling_rate)
    omega = 2 * np.pi * np.abs(f)
    v = 3.5 + 0.5 * np.random.rand()  # Add some randomness to velocity
    ...
    scattering = np.sqrt(t) * np.exp(-t / 40) * np.random.randn(len(t))  # Increased decay time
...
def apply_site_effects(data, station, component):
    ...
    amplification = 1 + 0.0002 * elevation  # Increased elevation effect

    t = np.arange(len(data)) / station.channels[0].sample_rate
    resonance_freq = 2 + np.random.rand() * 5  # Increased range of resonance frequencies
    resonance = 0.2 * np.sin(2 * np.pi * resonance_freq * t)  # Increased resonance effect

    freq = np.fft.rfftfreq(len(data), d=1/station.channels[0].sample_rate)
    smoothed_response = konno_ohmachi_smoothing(np.random.rand(len(freq)) + 1, freq, bandwidth=30, normalize=True)
...
def create_realistic_noise(npts, sampling_rate):
    t = np.arange(npts) / sampling_rate
    
    # Create a mix of white and colored noise
    white_noise = np.random.normal(0, 0.05, npts)
    colored_noise = np.cumsum(np.random.normal(0, 0.01, npts))
    colored_noise = colored_noise / np.max(np.abs(colored_noise)) * 0.1
    
    # Add some periodic noise (e.g., from human activities)
    periodic_noise = 0.05 * np.sin(2 * np.pi * 1 * t) + 0.03 * np.sin(2 * np.pi * 10 * t)
    
    # Combine all noise components
    total_noise = white_noise + colored_noise + periodic_noise
